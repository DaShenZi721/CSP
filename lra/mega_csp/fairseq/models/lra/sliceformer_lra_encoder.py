...
from fairseq.modules import (
    ...
    PositionalEmbedding,
    RealNumberEmbedding,
    SliceformerSentenceEncoderLayer,
    SliceformerAttention,
)
...
def init_bert_params(module):
    ...
    if isinstance(module, SliceformerAttention):
        module.q_proj.weight.data.normal_(mean=0.0, std=0.02)
        module.k_proj.weight.data.normal_(mean=0.0, std=0.02)
        module.v_proj.weight.data.normal_(mean=0.0, std=0.02)


class SliceformerLRAEncoder(nn.Module):
    """
    ...
    def __init__(
        ...
        self.layers.extend([
            self.build_sliceformer_sentence_encoder_layer(
    ...
    def build_sliceformer_sentence_encoder_layer(
        self,
        embedding_dim,
        ffn_embedding_dim,
        num_attention_heads,
        ...
        return SliceformerSentenceEncoderLayer(
    ...
    def forward(
        ...
        # new_seq_len = int(math.pow(2, math.ceil(math.log2(seq_len))))
        new_seq_len = self.max_seq_len
        zero_padding = torch.full((tokens.size(0), new_seq_len-seq_len), self.padding_idx, device=tokens.device)
        tokens = torch.cat([tokens, zero_padding], dim=1)
        # tokens torch.Size([24, 1792])
        # src_lengths 24 tensor([ 690, 1267,  554,  980, 1334, 1393,  524, 1688, 1037, 1792,  692, 1314,
        #     1560,  845, 1182, 1469,  715,  809, 1662,  938,  944,  834, 1582,  653],
        #    device='cuda:7')
        ...
        if self.embedding_type == 'sparse':
            ...
            if not self.traceable and not self.tpu and not padding_mask.any():
                padding_mask = None
            # B x T -> B x T x D
            # embedding之后，padding的token会被0向量填充
        ...
        # embed_positions之后，padding的token还是0向量
        ...
        for i in range(self.num_layers):
            if self.tie_layer_weights:
                ...
                j = i
            x, _ = self.layers[j](x, self_attn_padding_mask=padding_mask, src_lengths=src_lengths)
            if not last_state_only:
                inner_states.append(x)
