...
from fairseq.models.lra.sliceformer_lra_encoder import SliceformerLRAEncoder
...
class LRAModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--layer-type', choices=['transformer', 'luna', 'lstm', 'flash', 'mega', 'sliceformer'])
    ...
    def forward(self, sample):
        ...
        if not self.use_p:
            if self.layer_type in ['transformer', 'lstm', 'flash', 'mega', 'sliceformer']:
    ...
    def build_model(cls, args, task):
        ...
        # task = fairseq.tasks.long_range_arena.LRATextTask
...
@register_model_architecture('lra', 'sliceformer_lra_listop')
def sliceformer_lra_listop(args):
    ...
    args.layer_type = getattr(args, 'layer_type', 'sliceformer')
    base_architecture(args)
    
@register_model_architecture('lra', 'sliceformer_lra_imdb')
def sliceformer_lra_imdb_architecture(args):
...
@register_model_architecture('lra', 'sliceformer_lra_aan')
def sliceformer_lra_aan_architecture(args):
...
@register_model_architecture('lra', 'sliceformer_lra_pf32')
def sliceformer_lra_pf32(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
...
@register_model_architecture('lra', 'sliceformer_lra_cifar10')
def sliceformer_lra_cifar10(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 128)
    args.encoder_layers = getattr(args, 'encoder_layers', 1)
...
@register_model_architecture('lra', 'sliceformer_lra_pf128')
def sliceformer_lra_pf128(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'sliceformer')
