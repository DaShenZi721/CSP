...
class SliceformerAttention(nn.Module):
    ...
    def get_butterfly_matrix(self, v_len, d_v, device):
        ...
        while len(col_indices) < d_v:
            for factor_index in range(int(math.log(v_len, b))):
                ...
                col_index = (butterfly_matrix - torch.eye(v_len, device=device)) @ torch.arange(v_len, dtype=torch.float, device=device)
                col_index = torch.cat([col_index[-shift_offset:], col_index[:-shift_offset]])
                shift_offset = (shift_offset + len_sort_window) % v_len
    ...
    def forward(
        ...
        if self.butterfly_matrix is None:
            # self.butterfly_matrix = self.get_butterfly_matrix(tgt_len, embed_dim, value.device).unsqueeze(dim=0).long()
            self.butterfly_matrix = self.get_shift_matrix(tgt_len, embed_dim, value.device).unsqueeze(dim=0).long()
        if src_lengths is not None:
            butterfly_matrix = self.butterfly_matrix.repeat(bsz, 1, 1)
        ...
        if (
            ...
            return F.multi_head_attention_forward(
                ...
                butterfly_matrix=butterfly_matrix,
            )
        ...
        if saved_state is not None:
            ...
            key_padding_mask = SliceformerAttention._append_prev_key_padding_mask(
        ...
        attn_weights = SliceformerAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz)
