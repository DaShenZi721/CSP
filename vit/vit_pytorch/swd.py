...
class SWD21(nn.Module):
    ...
    def get_butterfly_matrix(self, v_len, d_v):
        ...
        #         col_index = (butterfly_matrix - torch.eye(v_len, device=self.device)) @ torch.arange(v_len, dtype=torch.float, device=self.device)
        ...
        for i in range(d_v):
            ...
            col_index = torch.cat([col_index[-shift_offset:], col_index[:-shift_offset]])
            shift_offset = (shift_offset + 2) % v_len
            col_indices.append(col_index)
    ...
    def forward(self, v):
        ...
        # out, _ = torch.sort(v, dim=1)
        
        # v_shifted = v.view(batch_size, -1, self.len_sort_window, d_v)
        ...
        out = torch.gather(v, 1, self.butterfly_matrix.repeat(batch_size, 1, 1))
        return out
...
class SWD22(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        v_sign = torch.sign(v - v_reverse)
        v_alter = v + (v_sign * v_abs_max).unsqueeze(dim=2)
        
        print('v_alter[0,:10,:10]')
        print(v_alter[0,:10,:10])
        ...
        v_sorted = v_sorted - (v_sign * v_abs_max).unsqueeze(dim=2)
        
        print('v_sorted[0,:10,:10]')
        print(v_sorted[0,:10,:10])
        1/0
        ...
        out = torch.gather(v_sorted, 1, self.butterfly_matrix.repeat(batch_size, 1, 1))
        return out